<div class="widget-first widget recent-posts">
    ...
    <div class="recent-post group">
        ...
        <div class="hentry-post group">
            ...
            <div class="text">
                <a href="{{ route('portfolios.show', ['aslias' => $portfolio->alias]) }}" title="{{ $portfolio->title }}" class="title">{{ $portfolio->title }}</a>
                <p>{{ str_limit($portfolio->text, 130) }}</p>
                <a class="read-more" href="{{ route('portfolios.show', ['aslias' => $portfolio->alias]) }}">&rarr; {{ Lang::get('ru.read_more') }}</a>
            </div>
        </div>
        @endforeach
        @endif
    ...
    <div class="recent-post recent-comments group">
     @foreach($comments as $comment)   
        <div class="the-post group">
            ...
            <a class="title" href="{{ route('articles.show', ['aslias' => $comment->article->alias]) }}">{{ $comment->article->title }}</a>
            <p class="comment">
                {{ $comment->text }}<a class="goto" href="{{ route('articles.show', ['aslias' => $comment->article->alias]) }}">&#187;</a>
            </p>
        </div>
     @endforeach       
    </div>
